...
type FileState =
  | { status: 'empty' }
  | { status: 'saved'; file: { name: string; size: number; public_id: string; secure_url: string; url?: string } };
...
export default function UploadPageClient() {
  ...
  useEffect(() => {
    if (resume?.file?.public_id && resume.file.name && resume.file.size && resume.file.secure_url) {
      setFileState({
        status: 'saved',
        file: {
          ...
          secure_url: resume.file.secure_url,
          url: resume.file.url ?? undefined, // optional
        },
      });
    }
  }, [resume]);
